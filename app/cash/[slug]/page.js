...
function makeFaq(page){return [
  {q:`${page.title} 신청은 회원가입이 필요한가요?`,a:'아니요. 별도 회원가입 없이 상품권 정보와 입금 계좌, 연락처를 입력해 신청할 수 있습니다.'},
  {q:`${page.title} 매입률은 어디에서 확인하나요?`,a:'페이지의 현재 매입 정보 영역에서 적용 중인 매입률을 확인할 수 있습니다. 매입률은 운영 상황에 따라 변경될 수 있으므로 신청 전에 표시된 비율을 확인해 주세요.'},
  {q:'상품권 번호를 잘못 입력하면 어떻게 하나요?',a:'신청 전 추가된 상품권 목록에서 잘못 입력한 항목을 삭제한 뒤 정확한 번호로 다시 등록해 주세요. 이미 접수가 완료된 경우에는 고객센터 확인이 필요할 수 있습니다.'},
  {q:'신청 후 처리 상태는 어디에서 확인하나요?',a:'메인 화면의 내 주문 조회에서 접수 시 입력한 전화번호와 조회 비밀번호를 입력하면 처리 상태를 확인할 수 있습니다.'}
]}

function jsonLd(data){return JSON.stringify(data).replace(/</g,'\\u003c')}
...
export default async function CashPage({params}){
  ...
  const faq=makeFaq(page);
  const url=`https://seoyo.kr/cash/${slug}`;
  const breadcrumbLd={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'사요 상품권',item:'https://seoyo.kr/'},{'@type':'ListItem',position:2,name:'상품권별 매입신청',item:'https://seoyo.kr/#rates'},{'@type':'ListItem',position:3,name:page.title,item:url}]};
  ...
  return <div className="sayo cashLanding" id="top">
    ...
    <main>
      <div className="shell cashBreadcrumb"><Link href="/">홈</Link><span>›</span><span>상품권별 매입신청</span><span>›</span><strong>{page.title}</strong></div>
      ...
      <section className="cashFaqSection"><div className="shell"><div className="cashApplyHeading"><p>FAQ</p><h2>{page.title} 자주 묻는 질문</h2><span>신청 전에 많이 확인하는 내용을 정리했습니다.</span></div><div className="faqListKsdl">{faq.map(x=><details key={x.q}><summary>{x.q}</summary><p>{x.a}</p></details>)}</div></div></section>
    </main>
