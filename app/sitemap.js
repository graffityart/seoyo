export default function sitemap(){
  ...
  return paths.map((path,i)=>({url:`${base}${path}`,lastModified:new Date(),changeFrequency:i===0?'daily':path.startsWith('/cash/')?'weekly':'monthly',priority:i===0?1:path.startsWith('/cash/')?.9:.7}));
}
