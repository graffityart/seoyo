    ...
    <div className="adminWrap">
      ...
      <main className="adminMain">
        ...
        <section className="adminPanel">
          <table className="orderTable"><thead><tr><th>접수번호</th><th>신청자</th><th>상품권</th><th>신청금액</th><th>예상입금</th><th>상태</th><th>접수일</th><th></th></tr></thead><tbody>{orders.map(o=><tr key={o.id}><td className="orderNo">{o.order_no}</td><td>{o.customer_name}</td><td>{o.product_names}{o.item_count>1?` ${o.item_count}건`:''}</td><td>{Number(o.requested_amount).toLocaleString()}원</td><td>{Number(o.expected_amount).toLocaleString()}원</td><td><span className={`status ${o.status}`}>{statusLabel[o.status]||o.status}</span></td><td>{kst(o.created_at)}</td><td><Link className="detailBtn" href={`/admin/orders/${o.id}`}>상세보기</Link></td></tr>)}</tbody></table>
          <div className="mobileOrders">{orders.map(o=><article className="mobileOrder" key={o.id}><div className="mobileOrderTop"><b className="orderNo">{o.order_no}</b><span className={`status ${o.status}`}>{statusLabel[o.status]||o.status}</span></div><h3>{o.customer_name}</h3><p>{o.product_names} · {o.item_count}건</p><p>{kst(o.created_at)}</p><div className="mobileOrderBottom"><div><span>신청금액</span><b>{Number(o.requested_amount).toLocaleString()}원</b></div><div><span>예상입금</span><b>{Number(o.expected_amount).toLocaleString()}원</b></div></div><Link className="detailBtn" href={`/admin/orders/${o.id}`}>신청 상세 확인</Link></article>)}</div>
        </section>
      </main>
    </div>
  </div>;
