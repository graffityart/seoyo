...
const productImages = {
  cultureland: '/images/products/01_cultureland.png',
  'online-culture': '/images/products/%23Uc628%23Ub77c%23Uc778%23Ubb38%23Ud654%23Uc0c1%23Ud488%23Uad8c.jpg',
  'cultureland-exchange': '/images/products/01_cultureland.png',
  teencash: '/images/products/02_tincash.png',
  'booknlife-book': '/images/products/04_booknlife.png',
  'booknlife-exchange': '/images/products/04_booknlife.png',
  'lotte-mobile': '/images/products/05_lotte.png',
  'google-gift': '/images/products/03_googleplay.png',
};
...
export default async function Home(){
  ...
  const safeProducts=products.map((p)=>({id:Number(p.id),name:p.name,slug:p.slug,default_rate:Number(p.default_rate),imageUrl:productImages[p.slug]||''}));
  ...
  return <div className="sayo" id="top">
    ...
    <main>
      <section className="heroKsdl"><div className="shell heroShell"><div className="heroCopyKsdl"><h1>쉽고 빠르다! 상품권 서비스</h1><p>고객님의 상품권을 빠르고 간편하게 확인합니다.<br/>별도의 복잡한 절차 없이 신청할 수 있습니다.</p><a href="#apply">상품권 현금교환</a></div><div className="orbitVisual"><div className="phoneMock"><div className="phoneLogo">SAYO</div><div className="phoneLine"></div><b>사요 상품권</b><small>상품권별 현재 매입률을 확인하세요</small></div>{rates.slice(0,5).map((r,i)=><span className={`orbitCard oc${i+1}`} key={r[0]}>{r[2]&&<img src={r[2]} alt={r[0]}/>}</span>)}</div></div></section>
