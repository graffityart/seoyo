...
export default async function Home(){
  ...
  return <div className="sayo" id="top">
    ...
    <main>
      <section className="heroKsdl"><div className="shell heroShell"><div className="heroCopyKsdl"><h1>쉽고 빠르다! 상품권 서비스</h1><p>고객님의 상품권을 빠르고 간편하게 확인합니다.<br/>별도의 복잡한 절차 없이 신청할 수 있습니다.</p><a href="#apply">상품권 현금교환</a></div><div className="orbitVisual" aria-label="사요 상품권 메인 배너 이미지 영역"><div className="phoneMock"><div className="phoneLogo">SAYO</div><div className="phoneLine"></div><b>사요 상품권</b><small>배너 이미지는 신규 이미지로 교체 예정</small></div>{rates.slice(0,5).map((r,i)=><span className={`orbitCard oc${i+1}`} key={r[0]}>{r[2] && <img src={r[2]} alt={r[0]}/>}</span>)}</div></div></section>
      ...
      <section id="faq" className="faqKsdl"><div className="shell"><div className="sectionTitle"><p>이용 전 확인</p><h2>자주 묻는 질문</h2><span>상품권 현금교환 신청 전 자주 문의하시는 내용을 확인해 주세요.</span></div><div className="faqListKsdl"><details><summary>회원가입이 필요한가요?</summary><p>별도 회원가입 없이 신청할 수 있습니다. 접수번호와 조회 비밀번호로 진행 상태를 확인할 수 있습니다.</p></details><details><summary>입금까지 얼마나 걸리나요?</summary><p>검수 및 운영 상황에 따라 순차적으로 입금 처리됩니다.</p></details><details><summary>매입하는 상품권은 어떤 것이 있나요?</summary><p>컬쳐랜드, 북앤라이프, 틴캐시, 롯데 모바일상품권, 구글 기프트카드 등을 취급합니다.</p></details><details><summary>최소 판매 금액이 있나요?</summary><p>최소 판매금액은 1만원 이상입니다.</p></details></div></div></section>
    </main>
