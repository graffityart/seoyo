...
export const SMS_DEFAULTS={
  ...
  sms_template_received_customer:'상품권 판매 신청이 정상 접수되었습니다. 확인 후 순차 처리됩니다.',
...
export async function sendNewOrderSms({orderId,orderNo,customerName,phone,requestedAmount,expectedAmount}){
  ...
  const vars={
    접수번호:String(orderNo||''),
    신청자:String(customerName||''),
    전화번호:customerPhone,
    신청금액:`${Number(requestedAmount||0).toLocaleString()}원`,
    입금금액:`${Number(expectedAmount||0).toLocaleString()}원`
  };
  const results=[];
  if(settings.sms_received_customer_enabled==='1'){
    try{
      results.push(await sendAndLogSms({orderId:Number(orderId),recipientType:'customer',phone:customerPhone,eventType:'received_customer',message:renderSmsTemplate(settings.sms_template_received_customer,vars)}));
    }catch(error){
      console.error('Customer receipt SMS failed',error);
      results.push({success:false,status:'failed',error:error?.message||String(error)});
    }
  }
  if(settings.sms_received_admin_enabled==='1'&&normalizePhone(settings.sms_admin_phone)){
    try{
      results.push(await sendAndLogSms({orderId:Number(orderId),recipientType:'admin',phone:settings.sms_admin_phone,eventType:'received_admin',message:renderSmsTemplate(settings.sms_template_received_admin,vars)}));
    }catch(error){
      console.error('Admin receipt SMS failed',error);
      results.push({success:false,status:'failed',error:error?.message||String(error)});
    }
  }
  return results;
